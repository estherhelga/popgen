...
Too high → Possible contamination (e.g., DNA from 2 people)
Too low → Possible inbreeding or genotyping failure
...
- Some chips are enriched for common SNPs → higher heterozygosity

- Others (like OmniExpress Plus) may focus more on tagging SNPs or rare variants → lower observed heterozygosity

2. This group might also include more genetically homogeneous individuals, e.g., people from a single ancestry → fewer heterozygous positions.
...
ggplot(qc_data, aes(x = HET_RATE, y = F_MISS)) +
  geom_point(alpha = 0.7) +
  coord_cartesian(ylim = c(0, 0.1) , xlim = c(0.15, 0.25)) +  # Zoom into meaningful y-range +
  theme_minimal() +
