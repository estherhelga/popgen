...
pca_eigenvectors <- read.table("final_pca_results_v2.eigenvec", header = FALSE, as.is = TRUE)
...
cat("Head of PCA Eigenvector Data (pca_eigenvectors):\n")
print(head(pca_eigenvectors))
cat("Dimensions:", dim(pca_eigenvectors), "\n\n")

N <- nrow(pca_eigenvectors)                     # number of individuals in your cohort
pca_eigenvalues <- read.table("final_pca_results_v2.eigenval", header = FALSE)
...
w <- fread("pca_with_wts.eigenvec.var", header = FALSE)
ggplot(w, aes(V2, abs(V5), colour = as.factor(V1))) +
  geom_point(size = .6) + facet_wrap(~V1, scales = "free_x", nrow = 4) +
  theme_bw() + labs(y="|loading|", x="Position") +
